...
def Fh(ymax, x):
    return (ymax-2*epsi)*(x <= ymax-2*epsi) + (ymax-2*epsi+h(x-ymax+2*epsi))*(ymax-2*epsi < x)

for k in range(K):
    Ymax[:, k] = np.max(X[:, :k+1], axis = 1)
    Y[:, k] = Fh(Ymax[:, k], X[:, k])
...
with tf.Graph().as_default():
    ...
    with sess.as_default():         
        ...
        input1 = tf.placeholder(shape = (None, K, 1), dtype = tf.float32)
        ...
        A1 = tf.Variable(initial_value = init(shape = [1, 1, N]), name = "A1", dtype = tf.float32)
        ...
        hidden1 = tf.reduce_sum(inp_int, -1) + A*inp_time + A1*input1 + b
        ...
        loss = tf.pow(tf.reduce_mean(tf.pow(tf.abs(y_pred - y_true), r)), 1/r)
